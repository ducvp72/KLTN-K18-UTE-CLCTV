...
import { Platform, ToastAndroid, PermissionsAndroid, Keyboard } from 'react-native';
import {
  renderSend, 
  renderVideo, 
  CustomtInputToolbar, 
  ...
  renderAvatar, 
  renderTime,
  renderCustomView, } from '../../components/MessageScreenRender'
...
export function Chat(props) {
  ...
  const getPermissions = async () => {
    ...
    if (!cameraGranted || !recordAudioGranted || !readExternalStorage || !writeExternalStorage 
  ...
  useEffect(() => {
    props.navigation.setOptions({
      ...
      header: ({ scene, previous, navigation }) => (
        <Header scene={scene} previous={previous} navigation={navigation} friendId={friendId} 
        groupName={groupName} groupType={groupType} groupId={groupId} callBuzz={callBuzz}/>
      ),
    });
    ...
    if (isValidMessage(message)) {
      ...
      if (message.text != 'null' && message.text != undefined) {
        transformedMessage.text = (CryptoJS.AES.decrypt(message.text, groupId)).toString(CryptoJS.enc.Utf8)
          ...
          transformedMessage.voice = (CryptoJS.AES.decrypt(message.voice, groupId)).toString(CryptoJS.enc.Utf8);
        } else if (message.file != 'null' && message.file != undefined) {
          transformedMessage.file = (CryptoJS.AES.decrypt(message.file, groupId)).toString(CryptoJS.enc.Utf8);
  ...
  const handleDocumentSelection = async() => {
    ...
    let result = await DocumentPicker.getDocumentAsync({ 
      ...
      if (response.type == 'success') {          
        ...
        var file = {
          name: (tempName[tempName.length - 1] == '.' ? tempName + type : tempName),
          size: size,
          uri: uri,
