...
function GroupInfoScreen(props) {
  ...
  const groupMemberList = () => {
    ...
    axios({
      ...
      .then((response) => {
        if (response.data) {
          setLoading(false);
          props.navigation.navigate("GroupMemberList", {
  ...
  const groupQR = () => {
    ...
    axios({
      ...
      .then((response) => {
        if (response.data) {
          setLoading(false);
          props.navigation.navigate("QRProfileScreen", {
            groupQR: response.data.qr,
  ...
  const changeNameGroup = async () => {
    if (group.admin.username !== auth.user.username) return;
    if (
      !createGroupName ||
      createGroupName == "" ||
        ...
        >
          ...
          <Portal>
            <ConfirmDialog
              ...
              toggleDialog={toggeConfirmDialog}
          ...
          <Portal>
            <ConfirmDialog
              ...
              toggleDialog={toggeLeaveDialog}
              deleteGroup={leaveGroup}
              title={t("common:leaveGroup")}
            />
          </Portal>
          <TouchableRipple>
            <View style={styles.subContainer}>
              <Text
                style={[
                  styles.leftText,
                  { flexGrow: 23, flex: 23, color: theme.colors.text },
                ]}
              >
                {t("common:avatar")}
              </Text>
              <Image
                style={[styles.avatar, { flexGrow: 5, flex: 5 }]}
                source={AppIcon.images.publicAvatar}
              />
              <Icon
                style={[
                  styles.chevronRight,
                  { color: theme.colors.text, flex: 2, flexGrow: 2 },
                ]}
                name="chevron-right"
              />
            </View>
          ...
          <TouchableRipple onPress={toggeDialog}>
            <View style={styles.subContainer}>
              <Text
                style={[
                  styles.leftText,
                  { flexGrow: 9, flex: 9, color: theme.colors.text },
                ]}
              >
                {t("common:info")}
              </Text>
              <Text
                ...
                style={[
                  styles.rightText,
                  { color: theme.colors.text, flex: 19, flexGrow: 19 },
                ]}
              >
                {group.groupName}
              </Text>
          ...
          <TouchableRipple onPress={groupMemberList}>
            <View style={styles.subContainer}>
              <Text
                style={[
                  styles.leftText,
                  { flexGrow: 20, flex: 20, color: theme.colors.text },
                ]}
              >
                {t("common:member")}
              </Text>
              <Text
                style={[
                  styles.rightText,
                  { color: theme.colors.text, flex: 8, flexGrow: 8 },
                ]}
              >
                {group.countMember}
              </Text>
