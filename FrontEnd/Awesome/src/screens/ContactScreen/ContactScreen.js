...
import { SCREEN_HEIGHT } from "../../utils/AppStyles";
...
export function ContactScreen(props) {
    ...
    return (
      ...
      >
        <View style={[styles.itemStyle, { backgroundColor: backgroundColor }]}>
          <Image
            source={{ uri: item.friends.avatar }}
  ...
  const onSearch = (searchText) => {
    ...
    let filteredName = trucks.filter((item) => {
      return (
        item.friends.fullname.toLowerCase().match(text) ||
        item.friends.username.toLowerCase().match(text)
      );
    });

    if (searchText) setSearchedList(filteredName);
    else setSearchedList(friendList);
  };
  ...
  return (
    <>
      ...
      <View
        style={[
          styles.friendReqContainer,
          { backgroundColor: backgroundColor, maxHeight: 60 },
        ]}
      >
        <Icon
          name="user-clock"
        ...
        />
        <Text
          style={[styles.friendReqText, { color: theme.colors.text }]}
          onPress={() => {
