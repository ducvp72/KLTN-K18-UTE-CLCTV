...
exports.parseStream = function(stream, callback) {
  ...
  stream.on("end", function() {
    ...
    if (!pngPixels) return error(new Error("Corrupt PNG?"));

    if (!pngTrailer) return error(new Error("Corrupt PNG?"));
  ...
  stream.on("data", function(data) {
    ...
    while (i !== len)
      switch (state) {
          ...
          if (len - i < 8 - off) {
            ...
            switch (buf.toString("ascii", 4, 8)) {
              ...
              case "tRNS":
                if (pngColorType !== 3)
                  return error(
                    new Error("tRNS for non-paletted images not yet supported.")
                  );
        ...
        case 6 /* IEND */:
          if (chunkLength !== 0) return error(new Error("Invalid IEND chunk."));
          else if (len - i < 4 - off) {
            off += len - i;
            i = len;
          } else {
        ...
        for (j = 0, x = 0; x !== pngWidth; ++x) {
          ...
          for (k = 0; k !== pngSamplesPerPixel; ++j, ++k)
            switch (pngBitDepth) {
              ...
              default:
                return error(
                  new Error("Unsupported bit depth: " + pngBitDepth)
                );
            }
